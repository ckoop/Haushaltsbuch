...
Zwei neue, kleine `pb.js`-Funktionen bedienen das: `listReserveRules(accountId)` (aktive `recurring_rules` mit `frequency = quarterly/yearly`, kein Filter auf `category` nötig — eine Regel ohne Kategorie matcht beim Rendern ohnehin keine echte Kategorie-ID) und `listRuleTransactions(ruleId)` (alle Buchungen mit passendem `import_hash`-Präfix). Beide werden **screen-lokal** in `Budgets.jsx` geladen (`useEffect`, Abhängigkeit nur `acc`, nicht Teil von `App.jsx`s zentralem `load()`) — betrifft nur diesen Screen, gleiches Prinzip wie der Depot-Chart-Hook oder der "Vorschlag aus Vormonat"-Button: nicht jeder Monats-/Tab-Wechsel in der ganzen App soll diese zusätzliche Abfrage mitschleppen.

**Bewusst nur in `Budgets.jsx` wirksam.** `spentByCat`/`real` in `App.jsx` (und damit Buchungen/Auswertung) bleiben unverändert — die tatsächliche Buchung im Fälligkeitsmonat ist dort weiterhin ganz normal sichtbar, das ist echtes Bargeld, das den Kontostand verändert hat. Ebenfalls unangetastet: das manuell gesetzte Budget (`budgets.amount_cents`) und die "Insgesamt verplant"-Vergleichsleiste — die Rücklage ist eine reine Zusatzanzeige neben der bestehenden Soll/Ist-Rechnung, kein neuer Bestandteil davon. Einzige sichtbare Änderung an einer bestehenden Zahl: die "bisher …"-Summe einer Kategorie mit Rücklage wird um die in diesem Monat tatsächlich entnommenen Rücklagen-Beträge bereinigt, damit sie nur die laufenden, nicht durch einen Topf abgefangenen Ausgaben zeigt.
